...
def optimal_sequence_search(net, config, logger, train_loader):
    with torch.no_grad():
        ...
        for i in range(len(train_loader.dataset)):
            # _index, _data, _label, _point_anno, _, _, _ = next(loader_iter)   #original_search.py_001
            _index, _data, _label, _point_anno, _, _, _, bkg_ann = next(loader_iter)    #modification_search.py_001
